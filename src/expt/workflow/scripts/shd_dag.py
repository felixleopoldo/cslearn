...
def shd_ldag(est_edges, true):
    """SHD between an edge set and the true CStree LDAG. Reversals count as 1, not 2."""
...
# Read the DAG/CPDAG produced by the phase-1 algorithm and extract its directed edges.
cpdag = pd.read_csv(snakemake.input["est"], index_col=0)
labels = [str(c) for c in cpdag.columns]
adj = cpdag.values.astype(int)
est_edges = [(labels[i], labels[j]) for i in range(len(labels)) for j in range(len(labels)) if adj[i, j] == 1]
